                    ...
                    <form action="{{ route('admin.categories.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-8">
                                ...
                                <div class="mb-3">
                                    <label for="description" class="form-label">Box Name</label>
                                    <select class="form-control @error('description') is-invalid @enderror" id="description"
                                        name="description">
                                        <option value="">-- Select Box --</option>
                                        <option value="Excellent" {{ old('description') == 'Excellent' ? 'selected' : '' }}>
                                            Excellent</option>
                                        <option value="Very Good" {{ old('description') == 'Very Good' ? 'selected' : '' }}>
                                            Very Good</option>
                                        <option value="Good" {{ old('description') == 'Good' ? 'selected' : '' }}>Good
                                        </option>
                                        <option value="Average" {{ old('description') == 'Average' ? 'selected' : '' }}>
                                            Average</option>
                                        <option value="Poor" {{ old('description') == 'Poor' ? 'selected' : '' }}>Poor
                                        </option>
                                    </select>
                                    @error('description')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                ...
                                @if ($boxes->count() > 0)
                                    <div class="mb-3">
                                        <label class="form-label">Associated Boxes</label>
                                        <div class="row">
                                            @foreach ($boxes as $box)
                                                <div class="col-md-4">
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="checkbox" name="boxes[]"
                                                            value="{{ $box->id }}" id="box_{{ $box->id }}"
                                                            {{ in_array($box->id, old('boxes', [])) ? 'checked' : '' }}>
